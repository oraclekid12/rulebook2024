...
Wounded Vampires are placed in an area to one side of the uncontrolled region, the torpor region. Any
...
modifiers during their actions. A vampire leaving torpor is no longer wounded.
...
> The cost to contest a card is 1 pool. The cost to contest a title is 1 blood.
...
You receive master phase actions during your master phase. By default, you receive only one master phase action. 

You may use a master phase action to play a master card, and some cards can give you alternate ways to use your master
phase actions. If other effects happen during your master phase, you choose the order in which these effects and your master phase actions are performed.
...
> REMINDER\
> When a Methuselah successfully plays a **trifle**, they gain an additional master phase action.\
> A Methuselah can gain only one master phase action from trifles in a given master phase; any subsequent trifles act like regular master cards.
...
> actions than normal. For an out-of-turn trifle, they gain a master phase action
in their next master phase. (see [[Master Cards](https://www.vekn.net/rulebook/2-card-types#master-cards)).


## **3. MINION PHASE**
*During your minion phase, you may command your minions to perform actions in your struggle for power over the other Methuselahs. Most of the activity in the game occurs in the minion phase. 
...
Actions can be used for several purposes like attacking other Methuselahs or their resources. Either attack your prey directly or use political power and negotiate. Minions can also bring permanent resources into play.*

Only **ready unlocked minions** can perform actions. Taking an action **locks** the
acting minion. If a minion manages to unlock (due to a card or an effect) during this
...
sections. Most actions are provided by action cards. However, there are two basic actions which do not require the play of action cards a vampire can perform: **bleed** or **hunt**. A ready ally may perform only one basic action which does not require the play of action cards: **bleed**.
...
> \
> The actions described below are either directed or undirected, depending on their targets. The target of an action constitutes which Methuselah(s) may attempt to block.\
> (see [**Who May Attempt to Block**](https://www.vekn.net/rulebook#who-may-attempt-to-block))

As a convenience, when a card describes an action that is typically directed at another Methuselah, the card's text will usually include a (D) symbol as a reminder that the action is typically directed. 
If not it is considered undirected.
...
fails (the acting vampire remains in torpor, and no cost is paid).\

> **REMINDER**\
> A wounded vampire is in the torpor region. A vampire in torpor may use action modifier cards. A vampire leaving torpor is no longer wounded.
...
different controllers.
- **Effect:**
	- If the action is successful, the vampire in torpor is
moved to the ready region. The rescued vampire does not lock or unlock
as a result of being rescued.
	- If the action is blocked, the acting vampire and the blocking minion enter combat as normal.

> **REMINDER**\
> A vampire rescued from torpor is no longer wounded.
...
same controller.
	- 0 stealth, if the acting vampire and the vampire in torpor have
different controllers.
- **Effect:** If the action is successful, the victim is diablerised. After that, a Blood Hunt is called.
...
After Diablerie, a Blood Hunt referendum is held. Diablerie is resolved as follows:
...
> REMINDER\
> The Blood Hunt referendum is not an action, so it cannot be blocked, and action modifiers and reaction cards cannot be played. Otherwise, this referendum is handled just like any other.\
> If the referendum is successful, the diablerist is burned.
...
the opportunity to play first. All cards must follow the same sequencing structures of all effects (see [**Sequencing**](https://www.vekn.net/rulebook/2-card-types#sequencing)). Action modifiers and reaction cards can be played at any time in this process as appropriate.

> The effect of an action modifier or a reaction card lasts until the action is resolved.


> **REMINDER**
> The same minion cannot play the same action modifier or reaction card more than once during each action.\
...
minion may decline to use the effect. In combat, there is no restriction on the use of a card with the same name.
